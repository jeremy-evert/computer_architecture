...
3. **Architecture-core consolidation** — planning done (Pass 3, 2026-08-25): read-only recon re-verified all 28 in-scope weeks-2–15 objects still 0/6 submitted and 0/6 graded, found and corrected a milestone-week error in the target design doc (Checkpoint 1 is Week 6 live, not Week 5), found Week 15 has no live object to consolidate, and produced full merged-assignment drafts for every week — see `docs/architecture-fall2026-target-course-design.md`'s "Pass 3" section for the complete reviewed recipe. **Execution deferred**, not because of a tooling gap (harbor's DELETE works) but because every real placement of a new merged assignment touches live assignment-group structure, which is gated behind the still-unsatisfied grade-impact-preview requirement (item 4 below). This is a genuine, distinct safety gate from Pass 2's shared-strand deletions (which added no replacement object to any group).
