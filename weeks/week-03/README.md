...
| Wednesday | Investigate / Break / Measure | Run the course laboratory health/probe path twice | reproducibility receipt |
| Friday | Explain / Defend | Separate controlled environment facts from uncontrolled host facts | bounded reproducibility claim |

## Important container boundary

Containers are a useful reproducibility tool, but **a container runtime is not required for the base Week 3 path**. The course laboratory's repository-local wrappers are the validated required path. A container image may be used later as optional enrichment once the executing platform proves it.
